@extends('layouts.appen')

@section('content') 
    <!-- Start Slider --> 
    <div id="slides-shop" class="cover-slides">
        <ul class="slides-container">
            @forelse ($slides as $s)
                <li class="text-center">
                    <img src="{{asset('assets/images/uploads/'.$s->image)}}" alt="">
                </li>
            @empty
                <li class="text-center">
                    <img src="https://media-exp1.licdn.com/dms/image/C4E1BAQFqTvDBFI20Ug/company-background_10000/0/1560318589119?e=2147483647&v=beta&t=NVS2LblK2qAwfrqmtRahlN2JvsMV8QBrgs29LEnUGEg" alt="">
                    <div class="container">
                        <div class="row">
                            <div class="col-md-12">
                                <h1 class="m-b-20"><strong>Welcome To <br> the syndicate</strong></h1>
                                <p class="m-b-40">The home of all computer scientists</p>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="text-center">
                    <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRGIdi2CbpWgofbmKFGSz9Tkv9Hlw2DQCB4lQ&usqp=CAU" alt="">
                </li>
                <li class="text-center">
                    <img src="https://img4.schoolandcollegelistings.com/331/720/2368997013317203.jpg" alt="">
                </li>
            @endforelse
        </ul>
        <div class="slides-navigation">
            <a href="#" class="next"><i class="fa fa-angle-right" aria-hidden="true"></i></a>
            <a href="#" class="prev"><i class="fa fa-angle-left" aria-hidden="true"></i></a>
        </div>
    </div>
    <!-- End Slider -->

	<div class="box-add-products">
		<div class="container">
			<div class="row">
				<div class="col-lg-4 col-md-4 col-sm-12">
					<div class="offer-box-products">
						<img class="syndicate-image" src="data:image/png;base64," alt="" />
					</div>
				</div>
				<div class="col-lg-8 col-md-8 col-sm-12">
					<div class="offer-box-products p-4">
						<h2 class="text-right"><u>SCSL:</u></h2>
                        <p>
                            <h3 class="text-right">The union includes persons who practice its profession and who fulfill the conditions specified in the Labor Law and in Decree /7993/ dated 3/4/1952 related to the organization of unions and in the union’s bylaws.</h3>
                            <br>
                            <h3 class="text-right">The aim of the union is limited to the following:</h3> 
                            <ul>
                                <li class="text-right">Protecting and encouraging the profession, raising its level, defending its interests, and working on its progress and development in all economic, industrial and commercial aspects.</li>
                                <li class="text-right">Taking care of the interests of the union members, defending their rights, and working to improve their level and their social, health, cultural, material and professional conditions in accordance with the laws and regulations in force.</li>
                                <li class="text-right">Establishing sports, social and cooperative consumption projects for the members of the union in accordance with the regulations in force.</li>
                                <li class="text-right">Strengthening the bonds of cooperation between employers and workers in this profession, finding a document of mutual understanding, and seriously striving to resolve any dispute that may arise between them through amicable means and negotiations stipulated in the laws in force.</li>
                                <li class="text-right">Provide professional and material assistance to its members within the capabilities available to it.</li>
                            </ul>
                        </p>
                        <button class="btn btn-primary details-btn">Details</button>
					</div>
				</div>
			</div>
		</div>
	</div>

     <div class="categories-shop">
        <div class="container">
            <h1 class="text-center m-2 heading"> <u>News:</u></h1>
            <div class="row">
                @forelse ($news as $n)
                    <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                        <div class="shop-cat-box">
                            <img class="img-fluid" src="{{asset('assets/images/uploads/'.$n->image)}}" alt="" style="margin-bottom: 75px;"/>
                            <a class="btn hvr-hover" href="#">
                                {{$n->name}}
                                <p>{{$n->text}}</p>
                            </a>
                        </div>
                    </div>
                @empty
                    <h1 class="text-center m-2">No News Yet</h1>
                @endforelse
            </div>
        </div>
    </div>

    <div class="box-add-products">
        <div class="container">
            <h1 class="text-center m-2 heading"> <u>Most frequent questions:</u></h1>
            <div class="questions">
                @forelse ($questions as $q)
                    <div class="row">
                        <div class="col-12">
                            <div class="question">
                                <h2><u>{{$q->question}}</u></h2>
                                <p>{{$q->answer}}</p>
                            </div>
                        </div>
                    </div>
                    @empty
                    <div class="row">
                        <div class="col-12">
                            <div class="question">
                                <p>No Questions Yet</p>
                            </div>
                        </div>
                    </div>
                    @endforelse
            <a href="#" class="btn btn-primary more">More...</a>
        </div>
    </div>

@endsection